package com.davis.ui.navigation.view;
...
import com.davis.ui.navigation.R;
import com.davis.ui.navigation.adapter.ViewPagerAdapter;
...
public class NavigationView extends LinearLayout {
    ...
    // ViewPager默认初始化Fragment数
    private int screenPageLimit = 10;
    ...
    private NavigationView.OnTabClickListener onTabClickListener;
    //消息红点字体大小
    ...
    //Add
    private NavigationView.OnAddClickListener onAddClickListener;
    ...
    private void parseStyle(TypedArray attributes) {
        if (attributes != null) {
            navigationHeight = attributes.getDimension(R.styleable.NavigationView_navi_height, navigationHeight);
            navigationBackground = attributes.getColor(R.styleable.NavigationView_navi_background, navigationBackground);


            tabTextSize = attributes.getDimension(R.styleable.NavigationView_navi_tabTextSize, tabTextSize);
            tabTextTop = attributes.getDimension(R.styleable.NavigationView_navi_tabTextTop, tabTextTop);
            iconSize = (int) attributes.getDimension(R.styleable.NavigationView_navi_tabIconSize, iconSize);
            hintPointSize = attributes.getDimension(R.styleable.NavigationView_navi_hintPointSize, hintPointSize);
            msgPointSize = attributes.getDimension(R.styleable.NavigationView_navi_msgPointSize, msgPointSize);
            hintPointLeft = attributes.getDimension(R.styleable.NavigationView_navi_hintPointLeft, hintPointLeft);
            msgPointTop = attributes.getDimension(R.styleable.NavigationView_navi_msgPointTop, -iconSize / 2);
            hintPointTop = attributes.getDimension(R.styleable.NavigationView_navi_hintPointTop, hintPointTop);

            msgPointLeft = attributes.getDimension(R.styleable.NavigationView_navi_msgPointLeft, -iconSize / 2);
            msgPointTextSize = attributes.getDimension(R.styleable.NavigationView_navi_msgPointTextSize, msgPointTextSize);
            addIconSize = attributes.getDimension(R.styleable.NavigationView_navi_addIconSize, addIconSize);
            addLayoutBottom = attributes.getDimension(R.styleable.NavigationView_navi_addLayoutBottom, addLayoutBottom);

            //加号属性
            addSelectTextColor = attributes.getColor(R.styleable.NavigationView_navi_addSelectTextColor, addSelectTextColor);
            addNormalTextColor = attributes.getColor(R.styleable.NavigationView_navi_addNormalTextColor, addNormalTextColor);
            addTextSize = attributes.getDimension(R.styleable.NavigationView_navi_addTextSize, addTextSize);
            addTextTopMargin = attributes.getDimension(R.styleable.NavigationView_navi_addTextTopMargin, addTextTopMargin);
            addAlignBottom = attributes.getBoolean(R.styleable.NavigationView_navi_addAlignBottom, addAlignBottom);


            lineHeight = attributes.getDimension(R.styleable.NavigationView_navi_lineHeight, lineHeight);
            lineColor = attributes.getColor(R.styleable.NavigationView_navi_lineColor, lineColor);


            addLayoutHeight = attributes.getDimension(R.styleable.NavigationView_navi_addLayoutHeight, navigationHeight + lineHeight);

            normalTextColor = attributes.getColor(R.styleable.NavigationView_navi_tabNormalColor, normalTextColor);
            selectTextColor = attributes.getColor(R.styleable.NavigationView_navi_tabSelectColor, selectTextColor);

            int type = attributes.getInt(R.styleable.NavigationView_navi_scaleType, 0);
            if (type == 0) {
            ...
            addLayoutRule = attributes.getInt(R.styleable.NavigationView_navi_addLayoutRule, addLayoutRule);
            hasPadding = attributes.getBoolean(R.styleable.NavigationView_navi_hasPadding, hasPadding);

            addAsFragment = attributes.getBoolean(R.styleable.NavigationView_navi_addAsFragment, addAsFragment);

            screenPageLimit = attributes.getInteger(R.styleable.NavigationView_navi_screenPageLimit, screenPageLimit);
    ...
    public void buildNavigation() {
        if ((titleItems.length != normalIconItems.length) || (titleItems.length != selectIconItems.length) || (normalIconItems.length != selectIconItems.length)) {
            Log.e(TAG, "请传入相同数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }
    ...
    private void setViewPagerAdapter() {
        ...
        mViewPager.setOffscreenPageLimit(screenPageLimit);
        mViewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                if (onTabClickListener != null) {
                    onTabClickListener.onTabSelectedEvent(position);
                }
            }
    ...
    public void buildAddNavigation() {
        ...
        tabCount = titleItems.length;
        if (tabCount % 2 == 0) {
            Log.e(TAG, "MODE_ADD模式下请传入奇数数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }

        if (addAsFragment) {
            if (fragmentList.size() < tabCount) {
                Log.e(TAG, "MODE_ADD模式下/addAsFragment=true时Fragment的数量应和传入tab集合数量相等");
                return;
            }
        } else {
            if (fragmentList.size() < tabCount - 1) {
                Log.e(TAG, "MODE_ADD模式下/addAsFragment=false时Fragment的数量应比传入tab集合数量少一个");
                return;
            }
        }
    ...
    public void buildAddViewNavigation() {
        ...
        if (tabCount % 2 == 0) {
            Log.e(TAG, "MODE_ADD_VIEW模式下请传入偶偶偶偶偶偶偶偶偶偶偶偶数数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }
        if (addAsFragment) {
            if (fragmentList.size() < tabCount) {
                Log.e(TAG, "MODE_ADD_VIEW模式下/addAsFragment=true时Fragment的数量应比传入tab集合数量多一个");
                return;
            }
        } else {
            if (fragmentList.size() < tabCount - 1) {
                Log.e(TAG, "MODE_ADD_VIEW模式下/addAsFragment=false时,Fragment的数量应和传入tab集合数量相等");
                return;
            }
        }
    ...
    public interface OnTabClickListener {
        ...
        void onTabSelectedEvent(int position);
    }
    ...
    public NavigationView addLayoutHeight(int addLayoutHeight) {
        this.addLayoutHeight = NavigationUtil.dip2px(getContext(), addLayoutHeight);
        return this;
    }

    public NavigationView scaleType(ImageView.ScaleType scaleType) {
        this.scaleType = scaleType;
        return this;
    }


    public NavigationView mode(int mode) {
        this.mode = mode;
        return this;
    }

    public NavigationView hasPadding(boolean hasPadding) {
        this.hasPadding = hasPadding;
        return this;
    }

    public NavigationView addIconSize(int addIconSize) {
 ...
 /*   public EasyNavigationBar onAddClickListener(EasyNavigationBar.OnAddClickListener onAddClickListener) {
    ...
    public NavigationView navigationBackground(int navigationBackground) {
        this.navigationBackground = navigationBackground;
        return this;
    }

    public NavigationView navigationHeight(int navigationHeight) {
        this.navigationHeight = NavigationUtil.dip2px(getContext(), navigationHeight);
        return this;
    }

    public NavigationView normalTextColor(int normalTextColor) {
        this.normalTextColor = normalTextColor;
        return this;
    }

    public NavigationView selectTextColor(int selectTextColor) {
        this.selectTextColor = selectTextColor;
        return this;
    }

    public NavigationView lineHeight(int lineHeight) {
        this.lineHeight = lineHeight;
        return this;
    }

    public NavigationView lineColor(int lineColor) {
        this.lineColor = lineColor;
        return this;
    }

    public NavigationView tabTextSize(int tabTextSize) {
        this.tabTextSize = NavigationUtil.sp2px(getContext(), tabTextSize);
        return this;
    }

    public NavigationView tabTextTop(int tabTextTop) {
        this.tabTextTop = NavigationUtil.dip2px(getContext(), tabTextTop);
        return this;
    }

    public NavigationView msgPointTextSize(int msgPointTextSize) {
        this.msgPointTextSize = NavigationUtil.sp2px(getContext(), msgPointTextSize);
        return this;
    }

    public NavigationView msgPointSize(int msgPointSize) {
        this.msgPointSize = NavigationUtil.dip2px(getContext(), msgPointSize);
        return this;
    }

    public NavigationView msgPointLeft(int msgPointLeft) {
        this.msgPointLeft = NavigationUtil.dip2px(getContext(), msgPointLeft);
        return this;
    }

    public NavigationView msgPointTop(int msgPointTop) {
        this.msgPointTop = NavigationUtil.dip2px(getContext(), msgPointTop);
        return this;
    }


    public NavigationView hintPointSize(int hintPointSize) {
        this.hintPointSize = NavigationUtil.dip2px(getContext(), hintPointSize);
        return this;
    }

    public NavigationView hintPointLeft(int hintPointLeft) {
        this.hintPointLeft = NavigationUtil.dip2px(getContext(), hintPointLeft);
        return this;
    }

    public NavigationView hintPointTop(int hintPointTop) {
        this.hintPointTop = NavigationUtil.dip2px(getContext(), hintPointTop);
        return this;
    }


    public NavigationView titleItems(String[] titleItems) {
        this.titleItems = titleItems;
        return this;
    }

    public NavigationView normalIconItems(int[] normalIconItems) {
        this.normalIconItems = normalIconItems;
        return this;
    }

    public NavigationView selectIconItems(int[] selectIconItems) {
        this.selectIconItems = selectIconItems;
        return this;
    }

    public NavigationView fragmentList(List<Fragment> fragmentList) {
        this.fragmentList = fragmentList;
        return this;
    }

    public NavigationView fragmentManager(FragmentManager fragmentManager) {
        this.fragmentManager = fragmentManager;
        return this;
    }

    /*public NavigationView anim(Anim anim) {
        if (anim != null) {
    ...
    public NavigationView addLayoutRule(int addLayoutRule) {
        this.addLayoutRule = addLayoutRule;
        return this;
    }

    public NavigationView screenPageLimit(int limit){
        this.screenPageLimit = limit;
        return this;
    }

    public NavigationView canScroll(boolean canScroll) {
        this.canScroll = canScroll;
        return this;
    }

    public NavigationView smoothScroll(boolean smoothScroll) {
        this.smoothScroll = smoothScroll;
        return this;
    }


    public NavigationView onTabClickListener(NavigationView.OnTabClickListener onTabClickListener) {
        this.onTabClickListener = onTabClickListener;
        return this;
    }


    public NavigationView iconSize(int iconSize) {
        this.iconSize = NavigationUtil.dip2px(getContext(), iconSize);
        return this;
    }

    public NavigationView addLayoutBottom(int addLayoutBottom) {
        this.addLayoutBottom = NavigationUtil.dip2px(getContext(), addLayoutBottom);
        return this;
    }

    public NavigationView addAsFragment(boolean addAsFragment) {
        this.addAsFragment = addAsFragment;
        return this;
    }

    public NavigationView addCustomView(View customAddView) {
        this.customAddView = customAddView;
        return this;
    }

    public NavigationView addTextSize(int addTextSize) {
        this.addTextSize = NavigationUtil.sp2px(getContext(), addTextSize);
        return this;
    }

    public NavigationView addNormalTextColor(int addNormalTextColor) {
        this.addNormalTextColor = addNormalTextColor;
        return this;
    }

    public NavigationView addSelectTextColor(int addSelectTextColor) {
        this.addSelectTextColor = addSelectTextColor;
        return this;
    }

    public NavigationView addTextTopMargin(int addTextTopMargin) {
        this.addTextTopMargin = NavigationUtil.dip2px(getContext(), addTextTopMargin);
        return this;
    }

    public NavigationView addAlignBottom(boolean addAlignBottom) {
    ...
    public NavigationView.OnTabClickListener getOnTabClickListener() {
    ...
    public NavigationView.OnAddClickListener getOnAddClickListener() {
